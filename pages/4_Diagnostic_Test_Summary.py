...
with build_col:
    ...
    with custom_cond_tier_tab:
        if len(st.session_state.custom_condition_list) == 0:
            ...
            _, col_0, col_1, _ = st.columns([0.2, 0.3, 0.3, 0.2])
            with col_0:
                inline_refresh_condition_df()          
            with col_1:  
                inline_apply_coustom_condition_df_to_tbls()
    ...
    with custom_test_tier_tab:
        if len(st.session_state.custom_test_tier_list) == 0:
            ...
            _, col_0, col_1, _ = st.columns([0.2, 0.3, 0.3, 0.2])
            with col_0:
                inline_refresh_test_df()

            with col_1:  
                inline_apply_custom_test_df_to_tbls()
